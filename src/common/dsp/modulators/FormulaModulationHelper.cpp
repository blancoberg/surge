                                                ...
                                                bool state[8])
{
    ...
    rec = [&rows, &es, &rec](const int depth, bool internal, bool parentIsUser, std::string filter,
                             showFilter show, int headerFlag) {
        ...
        if (lua_istable(es.L, -1))
        {
            ...
            auto guts = [&](const std::string &lab) {
                if (lua_isnumber(es.L, -1))
                {
                    ...
                    rows.back().headerFlag = headerFlag;
                }
                ...
                else if (lua_istable(es.L, -1))
                {
                    ...
                    if (depth < maxDepth)
                    {
                        ...
                        rows.back().headerFlag = headerFlag;
                        rec(depth + 1, internal, rows.back().isUserDefined, "", show, headerFlag);
                    }
                    else
                    {
    ...
    auto tables = {es.stateName, sharedTableName, es.stateName};
    std::string headers[3] = {" User variables", " Shared", " Built-in variables"};
    int ids[3] = {0, 1, 2};
    showFilter show[3] = {SHOW_USER, SHOW_ALL, SHOW_SYSTEM};
    ...
    for (const auto &t : tables)
    {
        ...
        // create header
        rows.emplace_back(0, headers[i], "");
        rows.back().isHeader = true;
        rows.back().headerFlag = ids[i];

        rec(0, false, false, filter, show[i], i);
    ...
    {
        if (((state[0] == false && rows[i].headerFlag == 0) ||
             (state[1] == false && rows[i].headerFlag == 1) ||
             (state[2] == false && rows[i].headerFlag == 2)) &&
            !rows[i].isHeader)
        {
...
{
    /*
    bool state[8] = {true, true, true, true, true, true, true, true};
    auto r = createDebugDataOfModState(es, "", state);
    std::ostringstream oss;
    for (const auto d : r)
    {
